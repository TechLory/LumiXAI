...
class CaptumOcclusionAttributor(BaseAttributor):
    ...
    whole token embedding at a time, and measures the resulting change in model output.
    This makes it fully model-agnostic in principle (any forward-callable model works),
    at the cost of one forward pass per token in the input (O(seq_len)) rather than a
    single backward pass — still cheap for the short prompts this framework targets.
